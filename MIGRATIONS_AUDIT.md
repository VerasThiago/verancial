...
**Severity:** High — **Resolved** by `migrations/007_fix_transactions_bank_id_type.sql`, confirmed by the e2e workflow's first real Postgres run.
...
At the time this finding was written, resolving it looked like an open behavioral question -- (a) `bank_id` should be a real UUID, or (b) it should stay a human-readable string and the Go-side `uuid` assumptions are wrong. That question was already answered, just not finished: `migrations/002_create_bank_accounts.sql` (merged separately, PR #22) had already switched `bank_accounts.id` and `user_bank_accounts.bank_id` to real `UUID` columns seeded with actual UUIDs (e.g. Scotiabank = `92f1aff9-03ae-4e6e-bde2-7799e849d181`), consistent with every `uuid`-typed assumption on the Go side. Only `transactions.bank_id` itself was never migrated to match. This was caught for real (not just inferred from reading the SQL) when the e2e test suite ran a report-processing request against an actual Postgres instance for the first time and hit `ERROR: operator does not exist: character varying = uuid (SQLSTATE 42883)` on every transaction query. `migrations/007_fix_transactions_bank_id_type.sql` converts the column with `ALTER COLUMN bank_id TYPE UUID USING bank_id::uuid`, completing what 002 started.
...
| `migrations/007_fix_transactions_bank_id_type.sql` | Converts `transactions.bank_id` from `VARCHAR(255)` to `UUID` (`USING bank_id::uuid`), matching every `?::uuid`-casting query in `postgresRepository/transaction.go`, the `gorm:"type:uuid"` model tag, and the `binding:"required,uuid"` HTTP validator -- and matching `bank_accounts.id`/`user_bank_accounts.bank_id`, already UUID since migration 002 | F4 (High) |
...
All new migration files are additive only (`CREATE INDEX IF NOT EXISTS`, `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, `DROP POLICY IF EXISTS` + recreate, or -- for 007 -- a type conversion that matches data already required to be UUID-shaped by the application code), safe to apply to an existing database. Unlike the rest of this audit, **migration 007 *has* been executed** -- against the e2e workflow's throwaway Postgres instance, where it was required to get any transaction query working at all.
