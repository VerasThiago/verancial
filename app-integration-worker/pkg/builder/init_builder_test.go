...
func TestServerBuilder_InitBuilder_PanicsOnUnreachableDB(t *testing.T) {
	aiwDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(aiwDir, "aiw.test.env"), []byte("AIW_PORT=8080\nASYNC_PROCESSING=false\n"), 0o600))
	...
	aiwEnvConfig := &shared.EnvFileConfig{Path: aiwDir, Name: "aiw.test", Type: "env"}
	sharedEnvConfig := &shared.EnvFileConfig{Path: sharedDir, Name: "shared.test", Type: "env"}

	assert.Panics(t, func() {
		new(ServerBuilder).InitBuilder(aiwEnvConfig, sharedEnvConfig)
...
func TestServerBuilder_InitBuilder_PanicsOnMissingConfigFile(t *testing.T) {
	...
	}, "InitBuilder should panic when the aiw env config file can't be read, not swallow the error")
}
